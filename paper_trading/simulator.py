...
class PaperTradingSimulator:
        ...
        for hour, data in enumerate(price_series):
            ...
            if not self.strategy.position:
                ...
                if analise['decisao']:
                    ...
                    print(f"🔴 [{timestamp.strftime('%d/%m %H:%M')}] VENDA @ R$ {price:,.2f}")
            ...
            if hour % 24 == 0 and hour > 0:
                dia = hour // 24
                capital_atual = self.capital if self.capital > 0 else self.positions[-1]['buy_price'] * self.positions[-1]['quantity']
    ...
    def generate_report(self):
        ...
        if self.trades:
            ...
            print(f"   Trades com lucro: {len([t for t in self.trades if t['profit_percentage'] > 0])}")
            print(f"   Trades com prejuízo: {len([t for t in self.trades if t['profit_percentage'] <= 0])}")
