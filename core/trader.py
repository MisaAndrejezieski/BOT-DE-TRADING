...
class TradingBot:
    ...
    def process_ticker(self, ticker: Dict):
        ...
        if not self.strategy.position:
            ...
            if analise['decisao']:
                logger.info(f"🎯 SINAL DE COMPRA! Confiança: {analise['confianca']:.1f}%")
            ...
            if analise['decisao']:
                logger.info(f"🎯 SINAL DE VENDA! Confiança: {analise['confianca']:.1f}%")
                for motivo in analise['motivos']:
                    logger.info(f"   • {motivo}")
    ...
    def _registrar_trade(self, tipo: str, preco: float, analise: Dict, resultado: Dict = None):
        """Registra um trade"""
        trade = {
