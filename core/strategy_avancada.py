...
class EstrategiaAvancada:
    ...
    def analisar_momento_compra(self, preco_atual: float) -> Dict:
        ...
        if len(self.price_history) > 1:
            queda = (preco_atual - self.price_history[-2]) / self.price_history[-2] * 100
            if queda <= -2.0:
                score += 0.3
                motivos.append(f"Queda de {queda:.1f}% (meta -2%)")
            else:
                return {'decisao': False, 'confianca': 0, 'motivo': f'Queda insuficiente: {queda:.1f}%'}
    ...
    def analisar_momento_venda(self, preco_atual: float, preco_compra: float) -> Dict:
        ...
        if lucro_atual >= 4.0:
            ...
            return {'decisao': False, 'confianca': 0, 'motivo': f'Lucro insuficiente: {lucro_atual:.1f}%'}
    ...
    def execute_sell(self, preco: float) -> Dict:
        ...
        result = {
            ...
            'sell_price': preco,
            'hold_time': (datetime.now() - self.position['buy_time']).total_seconds() / 3600
        }
